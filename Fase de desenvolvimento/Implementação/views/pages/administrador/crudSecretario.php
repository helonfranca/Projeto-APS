...
// Inclui a classe Secretario
include_once($_SERVER['DOCUMENT_ROOT'] . '/model/Secretario.php');
...
<body>
    <?php
    include_once($_SERVER['DOCUMENT_ROOT'] . '/model/Secretario.php'); //MUDAR HELON

    if (isset($_POST['cadastrar'])) {
        // Instancia um novo objeto Secretario
        $secretario = new Secretario();

        // Chama o método cadastrarSecretario() para cadastrar o novo secretário no banco de dados
        $dadosSecretario = array(
            'nome' => $_POST['nome'],
            'senha' => $_POST['senha'],
        ...
        $secretario->cadastrarSecretario($dadosSecretario);
        echo "<p>Secretário cadastrado com sucesso!</p>";
    }
    ?>
    <header>
        <div class="container">
    ...
    <section>
                    ...
                    <div class="fora-form">
                        <form method="post"">
                            ...
                            <div class="form-dados">
                                ...
                                <input type="date" id="dataNascimento" name="dataNascimento" placeholder="" required><br>

                                <label for="sexo">Sexo:</label></br>
                                <select name="sexo">
                                    <option value="M" checked <?php if (isset($_POST['sexo']) && $_POST['sexo'] === 'M') echo 'selected'; ?>>Masculino</option>
                                    <option value="F" <?php if (isset($_POST['sexo']) && $_POST['sexo'] === 'F') echo 'selected'; ?>>Feminino</option>
                                </select></br></br>
                            ...
                            <div style="text-align:center; margin-left: auto; margin-right: auto;">
                                <button type="submit" name="cadastrar" value="Cadastrar">Confirmar</button> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <div class="dentro-bloco">
            <h3>Gerenciar Secretario</h3>
        </div>

        <div class="bt-container">
            <button id="btn2" name="btn2">Adicionar novo secretario</button>
        </div>
                        ...
                        foreach ($listaSecretarios as $secretario) : ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <!-- Botão Verificar -->
                                    <button type="button" id="<?= $linha['Usuario_ID'] ?>" data-modal-id="modal1" class="btn1" onclick="visUsuario(id)">Verificar</button>


                                    <!-- Botão Editar -->
                                    <button type="button" data-modal-id="modal2" class="btn2" onclick="editUsuario(this)" id="<?= $linha['Usuario_ID'] ?>">Editar</button>

                                    <!-- Botão Remover -->
                                    <button type="button" data-modal-id="modal5" class="btn5" data-target="#modal5" onclick="setUsuarioIdRemover(<?= $linha['Usuario_ID'] ?>)">Remover</button>
